...
#ifndef CW32L012_LCD_SOFT_3SPI_PORT_H
#define CW32L012_LCD_SOFT_3SPI_PORT_H
...
/*------------------------------------------------------
** 在屏幕SPI通讯速率耐受较低时,建议使用"库函数"操作IO通讯
** 在屏幕SPI通讯速率耐受较高时,建议使用"寄存器"操作IO通讯
------------------------------------------------------*/
...
//-----SCL-----
//#define LCD_SCL_Clr() do{GPIO_WritePin(CW_GPIOA,GPIO_PIN_5,GPIO_Pin_RESET);}while(0)//库函数操作IO
//#define LCD_SCL_Set() do{GPIO_WritePin(CW_GPIOA,GPIO_PIN_5,GPIO_Pin_SET);}while(0)//库函数操作IO
#define LCD_SCL_Clr() do{CW_GPIOA->BRR = GPIO_PIN_5;}while(0)//直接寄存器操作,节省函数调用时间
#define LCD_SCL_Set() do{CW_GPIOA->BSRR = GPIO_PIN_5;}while(0)//直接寄存器操作,节省函数调用时间
...
//-----SDA-----
//#define LCD_SDA_Clr() do{GPIO_WritePin(CW_GPIOA,GPIO_PIN_7,GPIO_Pin_RESET);}while(0)//库函数操作IO
//#define LCD_SDA_Set() do{GPIO_WritePin(CW_GPIOA,GPIO_PIN_7,GPIO_Pin_SET);}while(0)//库函数操作IO
#define LCD_SDA_Clr() do{CW_GPIOA->BRR = GPIO_PIN_7;}while(0)//直接寄存器操作,节省函数调用时间
#define LCD_SDA_Set() do{CW_GPIOA->BSRR = GPIO_PIN_7;}while(0)//直接寄存器操作,节省函数调用时间
