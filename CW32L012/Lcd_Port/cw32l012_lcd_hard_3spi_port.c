...
void LCD_Send_1Dat(uint8_t dat)
{
	...
	LCD_CS_Clr();
	{
		send_lcd_spi_dat(0x100|dat);//向SPI发送缓冲器发送一个数据
	}
	wait_lcd_spi_txtemp_free();//等待SPI发送缓冲器为空
	send_lcd_spi_done();//等待SPI发送器空闲(发完)
	LCD_CS_Set();
...
void LCD_Send_nDat(uint8_t *p,uint16_t num)
{
	...
	{
		wait_lcd_spi_txtemp_free();//等待SPI发送缓冲器为空
		send_lcd_spi_dat(0x100|p[i++]);//向SPI发送缓冲器发送一个数据
	}
	wait_lcd_spi_txtemp_free();//等待SPI发送缓冲器为空
	send_lcd_spi_done();//等待SPI发送器空闲(发完)
	LCD_CS_Set();
...
void LCD_Port_Init(void)
{
  ...
  SPI_InitStructure.SPI_DataSize = SPI_DataSize_9b;                 // 帧数据长度为9bit
...
void LCD_Send_nCmd(uint8_t *p,uint16_t num)
{
	...
	//余下位按照数据发送
		...
		while(num-->0)
		{
			p++;
			wait_lcd_spi_txtemp_free();//等待SPI发送缓冲器为空
			send_lcd_spi_dat(0x100|*p);//向SPI发送缓冲器发送一个数据
		}
...
uint8_t LCD_Refresh(void)
{
	...
	for(y=0;y<SCREEN_HIGH;y++)
	{
		...
		for(x=0;x<SCREEN_WIDTH;)
		{
			...
			x++;
			wait_lcd_spi_txtemp_free();//等待SPI发送缓冲器为空
			send_lcd_spi_dat(0x100|(i>>8));//向SPI发送缓冲器发送一个数据
			wait_lcd_spi_txtemp_free();//等待SPI发送缓冲器为空
			send_lcd_spi_dat(0x100|(i&0x00ff));//向SPI发送缓冲器发送一个数据
		}
	}
...
uint8_t LCD_Refresh(void)
{
	...
	for(y=0;y<SCREEN_HIGH;y++)
	{
		...
		for(x=0;x<SCREEN_WIDTH;x++)
		{
			...
			#endif
			wait_lcd_spi_txtemp_free();//等待SPI发送缓冲器为空
			send_lcd_spi_dat(0x100|(lcd_driver.colour[c]>>8));//向SPI发送缓冲器发送一个数据
			wait_lcd_spi_txtemp_free();//等待SPI发送缓冲器为空
			send_lcd_spi_dat(0x100|(lcd_driver.colour[c]&0x00ff));//向SPI发送缓冲器发送一个数据
		}
	}
...
uint8_t LCD_Refresh(void)
{
	...
	for(ypage=0;ypage<GRAM_YPAGE_NUM;ypage++)
	{
		...
		for(i_dyna_count=0;i_dyna_count<PAGE_CRC_NUM;i_dyna_count++)
		{
			...
			//--------CRC算法校验--------
			//--方式1.库函数--
			//i_crc = CRC16_Calc_8bit(CRC16_IBM, i, (x_end-x_start)*LCD_COLOUR_BIT);//从*i开始校验, 连续校验(x_end-x_start)*LCD_COLOUR_BIT次
			//--方式2.寄存器--
			CW_CRC->CR = CRC16_IBM;//清空CRC计算值
			for(x=0;x<=(x_end-x_start)*LCD_COLOUR_BIT;x++) //从*i开始校验, 连续校验(x_end-x_start)*LCD_COLOUR_BIT次
			{
				CW_CRC->DR = *i++;
			}
			i_crc = CW_CRC->RESULT;//把校验值, 赋给i_crc
			//---------------------------
