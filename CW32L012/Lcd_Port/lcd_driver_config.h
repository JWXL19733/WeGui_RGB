...
//该工程支持四种工作模式, 内存充足情况下建议使用(2)
//(1)"全缓存全屏刷新",最原始的刷新模式,建议测试刷屏性能时使用
//(2)"全缓存动态刷新",最快的刷新模式, 全屏动画时与(1)的刷屏速度基本一致, 内存充足时建议使用
//(3)"页缓存全屏刷新",节省内存的原始刷新模式,建议测试刷屏性能时使用
//(4)"页缓存动态刷新",节省内存且刷得快得模式, 全屏动画时与(3)的刷屏速度基本一致, 内存不足时建议使用
...
/*------------------------------------------------------------------------------------
	...
	* |RES B10|  |3V3 3V3|       |LEFT | B15 | 左 |         UART_RX  A3 (暂未适配uart)
	* |SDA  A7|  |GND GND|       |RIGHT| B12 | 右 |         UART_TX  A2 (暂未适配uart)
	* |SCL  A5|                                       
	* |3V3 3V3|  
	* |GND GND|
-------------------------------------------------------------------------------------*/
...
//注:因主频96MHz较高, 软件模拟通讯时, 若屏幕点不亮, 尝试降低IO翻转速度, IO操作改成库函数操作, 或IO操作增加nop

//-------------------------1.选择一个屏幕通讯接口-----------------------------
#define _SOFT_3SPI  (0)//软件三线SPI驱动   对应文件cw32l012_lcd_soft_3spi_port.c 
#define _HARD_3SPI  (1)//硬件三线SPI驱动   对应文件cw32l012_lcd_hard_3spi_port.c
#define _SOFT_4SPI  (2)//软件四线SPI驱动   对应文件cw32l012_lcd_soft_4spi_port.c
#define _HARD_4SPI  (3)//硬件四线SPI0驱动   对应文件cw32l012_lcd_hard_4spi_port.c
//#define _DMA_4SPI   (4)//DMA四线SPI驱动   对应文件cw32l012_lcd_dma_4spi_port.c(官方未提供spi+dma的例程代码,没研究出来如何使用,暂未适配)
//#define _DMA_4SPI   (5)//DMA四线SPI驱动   对应文件cw32l012_lcd_dma_3spi_port.c(官方未提供spi+dma的例程代码,没研究出来如何使用,暂未适配)
#define _SOFT_I2C   (6)//软件IIC驱动 对应文件cw32l012_lcd_soft_i2c_port.c 
#define _HARD_I2C   (7)//硬件IIC驱动 对应文件cw32l012_lcd_hard_i2c_port.c
//#define _OTHER_PORT (8)//项目自定义
...
//#define SCREEN_WIDTH 64  //测试
//#define SCREEN_HIGH  64  //测试
...
//---------------------------3.1.设定I2C驱动--------------------------------
#if ((LCD_PORT == _HARD_I2C) || (LCD_PORT == _SOFT_I2C) || (LCD_PORT == _OTHER_PORT))
	...
	//#define I2C_BaudRate_Hz_x (2500000UL) //自定义
	//#define I2C_BaudRate_Hz_x (1000000UL) //1MHz快速+
	#define I2C_BaudRate_Hz_x (400000UL)  //400kHz快速
	//#define I2C_BaudRate_Hz_x (100000UL)  //100kHz快速(100k有问题,最低设置300k,疑似官方库有问题?)
	
#endif 

//-------------------------3.2.设定硬件SPI驱动-------------------------------
#if ((LCD_PORT == _HARD_4SPI) || (LCD_PORT == _DMA_4SPI) || (LCD_PORT == _HARD_3SPI) || (LCD_PORT == _OTHER_PORT) )
	//设置硬件SPI分频
	//CW32L010手册描述SPI最高设置24MHz 实测支持超频(96MHz/DIV2=48MHz)
	#define SPI_BAUDRATE_PCLK_DIVx       SPI_BAUDRATE_PCLK_DIV4 
#endif
...
//仅页缓存模式需要设置
	...
	#define GRAM_YPAGE_NUM (6)//自定义 最小取1 最大取(((SCREEN_HIGH+7)/8))
...
//实测cw32l012打开指令预取之后加速不明显, 已无需再打开ram加速
//此处保留, 作为移植使用
...
//------------编译-----------
	...
	#include "cw32l012_lcd_soft_3spi_port.h"
#elif (LCD_PORT == _HARD_3SPI)    //硬件三线SPI
	#include "cw32l012_lcd_hard_3spi_port.h"
#elif (LCD_PORT == _SOFT_4SPI) //软件四线SPI 
	#include "cw32l012_lcd_soft_4spi_port.h"
#elif (LCD_PORT == _HARD_4SPI) //硬件四线SPI 
	#include "cw32l012_lcd_hard_4spi_port.h"
#elif (LCD_PORT == _DMA_4SPI) //DMA四线SPI 
	#include "cw32l012_lcd_dma_4spi_port.h"
#elif (LCD_PORT == _SOFT_I2C)  //软件I2C   
	#include "cw32l012_lcd_soft_i2c_port.h"
#elif (LCD_PORT == _HARD_I2C)  //硬件I2C   
	#include "cw32l012_lcd_hard_i2c_port.h"
#elif (LCD_PORT == _DMA_IIC)  //DMA_IIC驱动
	#error ("cw32l012 dma iic driver is not supported yet")
#elif (LCD_PORT == _OTHER_PORT) //项目自定义驱动
	#include "cw32l012_lcd_other_port.h"
#endif
