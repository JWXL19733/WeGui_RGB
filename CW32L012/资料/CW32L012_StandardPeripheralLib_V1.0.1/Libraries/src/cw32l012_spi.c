...
/**
 * @brief 通过SPIx发送一个数据(4 ~ 16bit)，同时接收一个数据(4 ~ 16bit)
 *        该函数会等待发送缓冲区为空，然后发送数据，然后等待接收缓冲区非空，
 *        最后返回接收到的数据
 *
 * @param SPIx :SPIx外设  
 *        参数可以是:
 *           CW_SPI1、CW_SPI2、CW_SPI3
 * @param Data :待发送的数据
 * @return uint16_t :接收的数据
 */
...
{   
    ...
    // 等待发送缓冲区为空
    while((SPIx->ISR & SPIx_ISR_TXE_Msk) == 0);
    SPIx->DR = Data;
    // 等待接收缓冲区非空
    while((SPIx->ISR & SPIx_ISR_RXNE_Msk) == 0);
...
void SPI_Init(SPI_TypeDef* SPIx, SPI_InitTypeDef* SPI_InitStruct)
{
    ...
    REGBITS_MODIFY(SPIx->CR1, SPIx_CR1_MODE_Msk |\
                   ...
                   (SPI_InitStruct->SPI_BaudRatePrescaler << SPIx_CR1_BR_Pos)| SPI_InitStruct->SPI_Mode |\
...
void SPI_StructInit(SPI_InitTypeDef* SPI_InitStruct)
{
    ...
    SPI_InitStruct->SPI_BaudRatePrescaler = 0;     // PCLK/2,   SCK = PCLK/(2*(n+1))
