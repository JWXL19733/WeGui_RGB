/**
  ...
  * @version 1.0
  * @date    2024-08-15
  * @brief   本样例演示 OPA 被硬件 ADC 触发校正的功能。
...
int32_t main(void)
{
    ...
    char RAM_DisplayBuff[200];
    
    memset(RAM_DisplayBuff, 0 , 200);
...
static void ADC_Config(void)
{
    ...
    ADC_InitStruct.ADC_IN0.ADC_SampTime = ADC_SampTime262Clk;       // 1.2V采样时间要大于40us
    ADC_InitStruct.ADC_IN1.ADC_InputChannel = ADC_InputCH8;     // 配置序列通道1的模拟输入源为ADC_IN8(PB00)，OPA1OUT 
    ADC_InitStruct.ADC_IN1.ADC_SampTime = ADC_SampTime12Clk;       
    ADC_InitStruct.ADC_IN2.ADC_InputChannel = ADC_InputCH9;     // 配置序列通道2的模拟输入源为ADC_IN9(PB01) OPA1INP3
    ADC_InitStruct.ADC_IN2.ADC_SampTime = ADC_SampTime12Clk;       
...
{    
    ...
    // 使用ADC1Start信号触发OPA校正，需要配置ADC的SAM时间超过OPA的AZ时间
    OPA_AutoZeroTrig_Config(CW_OPA1, OPA_AZCLOCK_PCLK_DIV8, OPA_AZPERIOD_AZCLK64, OPA_AZTRIGSRC_ADC1START);
}
