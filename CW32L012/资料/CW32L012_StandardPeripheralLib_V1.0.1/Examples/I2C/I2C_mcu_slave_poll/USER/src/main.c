...
uint16_t u16LenReceive, u16LenSend;
uint32_t regBackup[5];
...
int32_t main(void)
{
...
#ifdef ADDR_SLAVE_10B_EN
    I2C_InitStruct.I2C_Addr0 = DEV_ADDR_SLAVE_10B;
    I2C_InitStruct.addressMatchMode = I2C_Addr0_10bit;
#else
...
#ifdef HSI48MHZ_EN
    I2C_SlaveInit(CW_I2C1, &I2C_InitStruct, 48000000UL); // 初始化模块
#else
    I2C_SlaveInit(CW_I2C1, &I2C_InitStruct, 8000000UL);  // 初始化模块
#endif

    I2C_SlaveCmd(CW_I2C1, ENABLE); // I2C模块使能

    if (I2C_SlavePollingFlag(CW_I2C1, I2C_IT_SLVBUSY_SLV) == I2C_Status_Success)
    {
    ...
    while (1)
    {
        ...
        // 等主机写数据
        if(I2C_Status_Success == I2C_SlaveReceiveDataFromMaster(CW_I2C1, u8RegAddr, I2C_REG_ADDR_16BIT, u8DataReceived, &u16LenReceive))
        {
            lenTemp = u16LenReceive;
            // 等主机读数据
            while (I2C_Status_Success != I2C_SlaveReceiveDataFromMaster(CW_I2C1, u8RegAddr, I2C_REG_ADDR_16BIT, u8DataReceived, &u16LenReceive))
            {
                I2C_SlaveClearFlag(CW_I2C1, ~CLEAR_IT_MASK_SLV);
            }
            I2C_SlaveSendDataToMaster(CW_I2C1, u8DataReceived, lenTemp);
            I2C_SlaveClearFlag(CW_I2C1, ~CLEAR_IT_MASK_SLV);
            PB09_TOG();
#if 0
#ifdef HSI48MHZ_EN
            FirmwareDelay(3000);
#else
            FirmwareDelay(500); // Ensure that all data has been sent from the slave.
#endif
#endif
...
uint8_t I2C_SlaveReceiveDataFromMaster(I2C_TypeDef *I2Cx, uint8_t *pu8RegAddr, I2C_SlaveRegAddrTypeDef u8RegAddrType, uint8_t *pu8Data, uint16_t *pu16Len)
{
    ...
    //清除I2C从机标志位
    I2C_SlaveClearFlag(I2Cx, ~CLEAR_IT_MASK_SLV);
    // 使能应答标志
    I2C_SlaveTxNAck(I2Cx, DISABLE);

#if 1
    // 查询等待 I2C_SISR.ADDR 变为 1
    while (I2C_SlavePollingFlag(I2Cx, I2C_IT_ADDR_SLV) != I2C_Status_Success)
    {
        ;
    }

#else
    if (I2C_SlavePollingFlag(I2Cx, I2C_IT_ADDR_SLV) != I2C_Status_Success)
    {
...
uint8_t I2C_SlaveReceiveSubAddressFromMaster(I2C_TypeDef *I2Cx, uint8_t *rxBuff, uint16_t *rxSize)
{
    uint32_t dataTemp;
    uint8_t *buf;
    uint32_t cntRx = 0;
    ...
    do
    {
        if(I2C_SlaveGetFlag(I2Cx, I2C_IT_RXNE_SLV))
        {
            ...
            *buf++ = (uint8_t)dataTemp;
            cntRx++;
        }

    }

    while (I2C_SlaveGetFlag(I2Cx, I2C_IT_RESTART_SLV) == 0);
