 ...
 ******************************************************************************/
uint8_t g_TxBuffer[LEN_DATA_RAM] = {0xff, 0x01, 0x02, 0x03, 0x04, 0x05, 0x60, 0x70, 0x80, 0x90, 0xa0, 0xb0, 0xc0, 0xd0, 0xe0, 0xf0};
uint8_t g_RxBuffer[2 + LEN_DATA_RAM];
...
void LPI2C_MasterTransferIT(void)
{
    ...
    masterXfer.dataSize = LEN_DATA_RAM;
    masterXfer.direction = I2C_MODE_WRITE;
    masterXfer.flags = kLPI2C_TransferDefaultFlag;
    masterXfer.slaveAddress = DEV_ADDR_SLAVE_7B;
    masterXfer.subaddress = RAM_ADDR_OFFSET_SLAVE;
    masterXfer.subaddressSize = 2;

    masterHandle.state = kIdleState;
    masterHandle.remainingBytes = LEN_DATA_RAM;
    masterHandle.buf = &u8StateMathine;
    masterHandle.userData = NULL;
    masterHandle.commandBuffer[0] = DEV_ADDR_SLAVE_7B | I2C_MODE_WRITE;
    masterHandle.commandBuffer[1] = RAM_ADDR_OFFSET_SLAVE >> 8;
    masterHandle.commandBuffer[2] = RAM_ADDR_OFFSET_SLAVE & 0x00FF;
...
void LPI2C_MasterReceiveIT(void)
{
    ...
    masterXfer.dataSize = 2 + LEN_DATA_RAM; //Include 2-byte subAddress.
...
int32_t main(void)
{
    ...
    while (1)
    {
        ...
        //检查Write/Read数据一致性
        if(0 == memcmp(g_TxBuffer, g_RxBuffer + 2, LEN_DATA_RAM))
        {
            //LED1亮/LED2灭  //PASSED
            PB09_SETHIGH();
            PB08_SETLOW();
        ...
        ++cntTemp;
        for (uint8_t i = 0; i < sizeof(g_TxBuffer); ++i)
        {
            g_TxBuffer[i] = cntTemp + i;
        }
