...
演示说明：
		  ...
		  传输完成后，比较TxBuffer2和RxBuffer1，如果数据一致，则LED1亮，否则LED1灭。
		  
		  -
		  #define  SPI_SLAVE  //从机模式
          从机查询方式（TXE）将TxBuffer2缓冲区中的数据写入DR寄存器。
          主机查询方式（RXNE）读取DR寄存器并存储到RxBuffer1缓冲区。
		  传输完成后，比较TxBuffer1和RxBuffer2，如果数据一致，则LED1亮，否则LED1灭。

硬件连接：
          SPI_MASTER SCK  (PA05)-- SPI_SLAVE SCK  (PA05)
          SPI_MASTER MISO (PA06)-- SPI_SLAVE MISO (PA06)
		  SPI_MASTER MOSI (PA07)-- SPI_SLAVE MOSI (PA07)
          SPI_MASTER CS   (PA09)-- SPI_SLAVE CS   (PA09)
