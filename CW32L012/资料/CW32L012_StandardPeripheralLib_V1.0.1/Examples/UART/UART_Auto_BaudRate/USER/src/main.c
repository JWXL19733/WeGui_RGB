...
{   
    ...
    while (1)
    {
        //配置UART, 上位机发送字符'C'以识别波特率
        UART_Configuration();
        printf("The baud rate has been automatically matched.\r\n");
        printf("Please reset the baud rate.\r\n");
    }
}
...
void UART_Configuration(void)
{
    ...
    UART_EnableAutoBaudRateDetection(DEBUG_UARTx, UART_AUTO_BAUD_RATE_MODE_1);
    while (UART_AutoBaudRateDetectionComplete(DEBUG_UARTx) != TRUE);
    UART_DisableAutoBaudRateDetection(DEBUG_UARTx);

    UART_InitStructure.UART_BaudRate = DEBUG_UART_UclkFreq / UART_GetDetectedBaudRate(DEBUG_UARTx);
