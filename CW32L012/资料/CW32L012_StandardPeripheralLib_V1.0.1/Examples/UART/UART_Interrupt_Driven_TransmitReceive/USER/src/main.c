...
int32_t main(void)
{
    ...
    {
        //发送
        UART_TransmitInterrupt(DEBUG_UARTx, TxRxBuffer, sizeof(TxRxBuffer));
        //接收
        UART_ReceiveInterrupt(DEBUG_UARTx, TxRxBuffer, sizeof(TxRxBuffer));
        while (rxComplete != TRUE);
    }
}
...
void UART3_IRQHandlerCallBack(void)
{
    //接收
    if (UART_GetITStatus(DEBUG_UARTx, UART_IT_RC) != RESET)
    {
        ...
        {
            *rxDataPtr = UART_ReceiveData(DEBUG_UARTx);
            rxDataLen--;
            rxDataPtr++;
        }
    ...
    //发送
    if (UART_GetITStatus(DEBUG_UARTx, UART_IT_TC) != RESET)
    {
        UART_ClearITPendingBit(DEBUG_UARTx, UART_IT_TC);
        if (txDataLen == 0)
        {
            txComplete = TRUE;
        }
        else
        {
            txDataLen--;
            txDataPtr++;
            UART_SendData(DEBUG_UARTx, *txDataPtr);
        }
