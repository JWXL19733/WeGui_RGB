...
int32_t main(void)
{
    ...
    UART_TransmitDMA(DEBUG_UARTx, DMA_CHANNEL_1, TxRxBuffer, 200);
    while (CW_DMA->ISR_f.TC1 == 0);   // 等待DMA发送完成

    while (1)
    {
        UART_ReceiveDMA(DEBUG_UARTx, DMA_CHANNEL_2, TxRxBuffer, 10);
        while (CW_DMA->ISR_f.TC2 == 0);    // 等待DMA接收到足够的数量
        
        UART_TransmitDMA(DEBUG_UARTx, DMA_CHANNEL_1, TxRxBuffer, 10);
        while (CW_DMA->ISR_f.TC1 == 0);   // 等待DMA发送完成
