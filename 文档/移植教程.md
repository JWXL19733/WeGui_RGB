...
#endif
```
*下方是动态细分调整,此处不描述具体作用,移植前可随意设置, 建议取6~8
```-------------------------1.5.6设置动态刷新页细分数量--------------------------------
//每页连续横向扫描n个像素校验一个crc
...
```
* 如果屏幕是彩色屏, 支持选择1~3位色显示, 为了节省ram可设置1, 空间充足可选择2~3
```
//-----------------------1.5.12设置色彩深度---------------------------
...
#endif
* lcd_gray_port()函数是灰度屏幕接口移植函数, 从第page页的x0开始刷屏刷到x1,每次调用最多只刷当前1个页, 不会换行换page
* 需要根据OLED储存格式转换后输出到屏幕(暂无例程参考)
```
/*--------------------------------------------------------------
...
```
* 下方是自定义交互接口,可先关闭交互接口
* 其中wegui_port_task(stick)会放到main while中持续执行,stick用于回去时间基准,单位是ms,接口放到do{}while(0)中
* wegui_port_ms_irq()会放到1ms stick中断里执行,快速响应的接口,接口放到do{}while(0)中
* wegui_port_init()会放在初始化时执行一次,接口放到do{}while(0)中
