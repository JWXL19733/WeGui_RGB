...
/*--------------------------------------------------------------
  * 名称: void wegui_uart_tx_1dat(uint8_t dat)
  * 传入: 
  * 返回: 
  * 功能: 向uart发送一个数据
  * 说明: 
----------------------------------------------------------------*/
void wegui_uart_tx_1dat(uint8_t dat)
{
		...
		switch (deal_code)
		{
				...
				switch(dat)//控制码
				{
					...
					case 0x30://[0]0x00 [1]0x30 [2]0xCF [3]0xAA软件复位 [复位前发送: 头信息 0x01 0x30 0x01]
					{
						...
						if(read_rxRing_buff() != 0xAA){break;}
						{
							...
							__disable_fault_irq();
							wegui_enter_menu(&m_App_MusicPlayer);NVIC_SystemReset();
						}
					}break;
			...
			case DEAL_REQUEST:
			{
				switch(dat)
				{
					...
					{
						char i2[]=STR_WEGUI_VERSION;
						uint8_t i[]={0x78,0x00,(2+sizeof(i2)/sizeof(char)),0x81,0x05};
						Wegui_UartSend_nDat((uint8_t*)i,sizeof(i)/sizeof(uint8_t));
						Wegui_UartSend_nDat((uint8_t*)i2,sizeof(i2)/sizeof(char));
					}break;
