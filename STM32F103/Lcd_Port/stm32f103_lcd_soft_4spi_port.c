...
static void flash_send_1Byte(uint8_t dat);
/*--------------------------------------------------------------
...
uint8_t LCD_Refresh(void)
{
	...
	for(ypage=0;ypage<GRAM_YPAGE_NUM;ypage++)
	{
		...
		for(x=0;x<(SCREEN_WIDTH*LCD_COLOUR_BIT);x++) //从*i开始校验, 连续校验(x_end-x_start)*LCD_COLOUR_BIT次
		{
			CRC->DR = *i++;//i[x];
		}
