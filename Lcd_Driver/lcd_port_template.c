...
/*--------------------------------------------------------------
  ...
  * 说明: 无需精准
----------------------------------------------------------------*/
__attribute__((weak)) void lcd_delay_ms(volatile uint32_t ms)
{
		volatile uint16_t i;
		while (ms--)
		{
...
----------------------------------------------------------------*/
__attribute__((weak)) void lcd_bl_on(void)
{
...
----------------------------------------------------------------*/
__attribute__((weak)) void lcd_bl_off(void)
{
	while(1)//需要移植 参考stm32f103例程
	{
		;
...
----------------------------------------------------------------*/
__attribute__((weak)) inline uint8_t lcd_is_busy(void)
{
	return 0;
}
...
/*--------------------------------------------------------------
  * 名称: lcd_port_init()
  * 功能: 屏幕接口初始化
  * 说明: SPI 背光IO等 weak类型 需要改写
----------------------------------------------------------------*/
__attribute__((weak)) void lcd_port_init(void)
{
	while(1)//需要移植 参考stm32f103例程
	{
		;
...
/*--------------------------------------------------------------
  ...
  * 传入1: dat待发送的命令
  * 功能: 向屏幕发送1个命令 weak类型 需要改写
----------------------------------------------------------------*/
__attribute__((weak)) void lcd_send_1Cmd(uint8_t dat)
{
	while(1)//需要移植 参考stm32f103例程
	{
		;
...
/*--------------------------------------------------------------
  ...
  * 传入1: *p待发送的数组指针
  * 传入2: num发送数量
  * 功能: 向屏幕发送num个命令 weak类型 需要改写
----------------------------------------------------------------*/
__attribute__((weak)) void lcd_send_nCmd(uint8_t *p,uint16_t num)
{
	while(1)//需要移植 参考stm32f103例程
	{
		;
...
/*--------------------------------------------------------------
  ...
  * 传入1: *p数组指针
  * 传入2: num发送数量
  * 功能: 向屏幕发送1个数据 weak类型 需要改写
----------------------------------------------------------------*/
__attribute__((weak)) void lcd_send_1Dat(uint8_t dat)
{
	while(1)//需要移植 参考stm32f103例程
	{
		;
...
/*--------------------------------------------------------------
  ...
  * 传入1: *p数组指针
  * 传入2: num发送数量
  * 功能: 向屏幕发送num个数据 weak类型 需要改写
----------------------------------------------------------------*/
__attribute__((weak)) void lcd_send_nDat(uint8_t *p,uint16_t num)
{
	while(1)//需要移植 参考stm32f103例程
	{
		;
